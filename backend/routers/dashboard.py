...
from backend.models import ProcessedEmail, Stats
...
def get_activity(limit: int = 50, session: Session = Depends(get_session)):
    statement = select(ProcessedEmail).order_by(ProcessedEmail.processed_at.desc()).limit(limit)
    return session.exec(statement).all()

@router.get("/stats")
def get_stats(session: Session = Depends(get_session)):
    # Simple aggregation for now
    total_emails = session.exec(select(ProcessedEmail)).all() # Potentially slow if many
