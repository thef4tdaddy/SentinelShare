<script lang="ts">
	import { CheckCircle, AlertCircle, RefreshCw, ThumbsUp, ThumbsDown } from 'lucide-svelte';
	...
	function getModalTitle(): string {
		if (!selectedEmail) return 'Email Details';
		if (selectedEmail.status === 'ignored') return 'Forward Ignored Email';
		if (selectedEmail.status === 'forwarded' || selectedEmail.status === 'blocked')
			return 'Change Email Status';
	...
	<Modal {isOpen} {onClose} title={getModalTitle()}>
		...
		{:else if selectedAnalysis}
			<div class="mb-4">
				...
				>
					Final Decision: {selectedAnalysis.analysis.final_decision ? 'RECEIPT' : 'NOT A RECEIPT'}
				</div>
			</div>
		{/if}
