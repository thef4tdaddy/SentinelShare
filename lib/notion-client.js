    ...
    try {
      ...
      for (const page of response.results) {
        ...
        const rule = {
          ...
          forwardTo: properties['Forward To']?.email || '',
          priority: properties.Priority?.number || 999,
          status: properties.Status?.select?.name || 'Inactive'
        }
        
        if (rule.status === 'Active' && (rule.emailPattern || rule.subjectPattern) && rule.forwardTo) {
          rules.push(rule)
        }
      }
    ...
    try {
      await this.notion.pages.create({
        ...
        properties: {
          ...
          'Reason': {
            rich_text: [{ text: { content: this.truncateText(`Rule: ${rule.name} -> ${rule.forwardTo}`, 100) } }]
          },
          'Date': {
            date: { start: new Date().toISOString() }
          }
